...
#Directories
dir <- file.path("C:", "Users", "jacqu", "OneDrive - University Of Cambridge", "PhD", "Thesis", "Chapter 4", "Cydar")
out_dir <- file.path("C:", "Users", "jacqu", "Dropbox", "NI Submission 2021", "figure images", "Fig1")

#Import data
count_dat <- readRDS(file.path(dir, "count_dat_Bcell10000_10downsample_filter50_range.rds")) #downsample 10, filter 10 B cells only (10,000 events per sample)
...
design.use <- model.matrix(~ 0 + conditions) ##Comparing everything all together
colnames(design.use) <- levels(conditions)
y_design <- estimateDisp(y_keep, design.use) #Goes with plotBCV

#fit for dispersion using QL
fit.dispersion <- glmQLFit(y_design, design = design.use, robust = TRUE) #no offset because same size 30000 otherwise offset = log(count_keep$totals)
...
#As copied from Stats of Counts above, dat_counts gives counts for all only
...
#### Fig1b (raw)
for (i in 1:3){
  count_use <- tissues_count[,i]
  dat.with.colours <- data.frame(umap.dat, count_use)
  
  pdf(file.path(out_dir, paste0("HypersphereLogCounts_UMAP_", tissues_name[i], ".pdf")), width=7, height=5)
  
  a <- ggplot(data = dat.with.colours %>% arrange(is.sig), aes(x = UMAP1, y = UMAP2)) +
    geom_point(aes(fill = count_use, size = is.sig, colour = is.sig), pch = 21) + 
    theme_pubr() +
    scale_fill_viridis(trans = "log10", limits = c(0.5, 2000)) + 
    scale_colour_manual(values = c("transparent", "black")) + 
    scale_size_manual(values = c(0.5, 7)) +
    labs(x = "UMAP_1", y = "UMAP_2", fill = "Median Count", title = paste0(tissues_name[i]), size = "p < 0.05") +
    theme(legend.position="right", text = element_text(size = 20), axis.ticks=element_blank(), axis.text=element_blank()) +
    guides(colour = FALSE)
...
#### Fig 1d (overlay cluster onto UMAP)
